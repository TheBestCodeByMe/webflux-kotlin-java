...
import org.dbs.validator.Field.SOME_USER_FIRST_NAME
import org.dbs.validator.Field.SOME_USER_ID
import org.dbs.validator.Field.SOME_USER_LAST_NAME
        ...
        buildGrpcResponse({
            it.run {
                ...
                fun validateUserV1Credentials(): Mono<RAB> = run {
                    validateExpiredClaim(CL_USER_ID, SOME_USER_ID) { userV1Id.hold(it.toLong()) }
                    validateExpiredClaim(CL_USER_FIRST_NAME, SOME_USER_FIRST_NAME) { userFirstName.hold(it) }
                    ...
                    validateExpiredClaim(CL_USER_LAST_NAME, SOME_USER_LAST_NAME) { userLastName.hold(it) }
                    validateClaim(CL_USER_LOGIN, SOME_LOGIN_USER) { userV1Login.hold(it) }
